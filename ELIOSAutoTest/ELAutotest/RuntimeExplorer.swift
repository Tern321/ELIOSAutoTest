...
class RuntimeExplorer: NSObject {
    ...
    public static func classes(conformToProtocol proto: Protocol) -> [AnyClass] {
        let classes = self.allClasses().filter { aClass in
            var subject: AnyClass? = aClass
            while let aClass = subject {
                if class_conformsToProtocol(aClass, proto) { print(String(describing: aClass)); return true } // wtf print
